...
void PrintWindowText(HWND hwnd, char* info)
{
	if (hwnd)
	{
		TCHAR title[256];
...
void PrintForegroundWindow()
{
	HWND hwnd = GetForegroundWindow(); 
	PrintWindowText(hwnd, "foreground window");
}

void PrintFocusedWindow()
{
	HWND hwnd = GetFocus(); 
	PrintWindowText(hwnd, "focused window");
}
...
void CreateConsole()
{
	// Make output work in console if we are not in cygiwn.sss
	// 将当前进程绑定到父进程的console
	if (!AttachConsole(ATTACH_PARENT_PROCESS))
		AllocConsole();  // 绑定失败，创建新的console
